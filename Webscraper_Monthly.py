...
def get_url_paths(url, ext=''):
    ...
    return CSV_links
def main():
    url = "https://www.emi.ea.govt.nz/Wholesale/Datasets/DispatchAndPricing/FinalEnergyPrices/ByMonth"
    ext = '.csv'
    ...
    for file in result:
        ...
        if not os.path.exists(f'C:/Users/bryce/OneDrive/Desktop/Personal Projects Desktop/Electricity Data Monthly/{f_name}.csv'):
            if f_name[0].isdigit(): # Need to filter out the daily prices
                # The file does not exists. Can save
                r = requests.get(file)
                # r = requests.get(file, auth=HTTPBasicAuth('username', 'password'))
                with open(f'C:/Users/bryce/OneDrive/Desktop/Personal Projects Desktop/Electricity Data Monthly/{f_name}.csv', 'wb') as f:
                    f.write(r.content)
